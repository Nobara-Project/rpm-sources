...
Patch1:         0001-enable-nvidia-drm.modeset-1-as-default.patch
Patch2:         0001-enable-nvidia-drm.fbdev-1-as-default.patch
Patch3:         0001-nvidia-drm-hotplug-workqueue.patch

# get the needed BuildRequires (in parts depending on what we build for)
BuildRequires:  kmodtool

# kmodtool does its magic here
%{expand:%(kmodtool --target %{_target_cpu} --repo nobara-appstream --kmodname %{name} %{?buildforkernels:--%{buildforkernels}} %{?kernels:--for-kernels "%{?kernels}"} 2>/dev/null) }
...
kmodtool  --target %{_target_cpu}  --repo nobara-appstream --kmodname %{name} %{?buildforkernels:--%{buildforkernels}} %{?kernels:--for-kernels "%{?kernels}"} 2>/dev/null
