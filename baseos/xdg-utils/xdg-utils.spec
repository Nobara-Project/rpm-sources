...
mv %buildroot/%{_bindir}/xdg-open %buildroot/%{_bindir}/xdg-open-real

cat << 'EOF' > %buildroot/%{_bindir}/xdg-open
#!/bin/sh

# Hack to prevent libextest LD_PRELOAD from being forwarded when steam uses xdg-open
# Use the parent PID to find the parent process name
if [ "$(ps -p $PPID -o comm=)" = "steam" ]; then
    unset LD_PRELOAD
fi
...
chmod 755 %buildroot/%{_bindir}/xdg-open
