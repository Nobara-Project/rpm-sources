...
Requires(post): dkms >= 3.4.3
Requires(post): drm-awaiter >= 1-2
Requires(preun): dkms >= 3.4.3
Requires(preun): drm-awaiter >= 1-2
# Lets CFHDB skip its redundant rebuild only with the updated RPM scriptlets.
Provides:       nvidia-dkms-batched-initramfs = 1
...
%post
# Request first so a partially failed installation still gets image cleanup.
%{_libexecdir}/drm-awaiter-initramfs request || exit $?
dkms add -m %{dkms_name} -v %{version} --rpm_safe_upgrade
add_status=$?
# DKMS returns 3 when this version is already registered (RPM reinstall).
if [ "$add_status" -ne 0 ] && [ "$add_status" -ne 3 ]; then
    echo "Failed to register NVIDIA %{version} with DKMS." >&2
    exit 1
fi

failed=0
found=0
# DKMS's post_transaction is global. Override it only for these invocations;
# the shared RPM transaction trigger performs the final image rebuild instead.
for ksrc in /usr/src/kernels/*; do
    kver="${ksrc##*/}"
    [ -d "$ksrc" ] && [ -d "/lib/modules/$kver" ] || continue
    found=1
    echo "Building NVIDIA %{version} for $kver..."
    if ! dkms build -m %{dkms_name} -v %{version} -k "$kver" --force --kernelsourcedir "$ksrc" --directive post_transaction=; then
        echo "NVIDIA build failed for $kver; see /var/lib/dkms/%{dkms_name}/%{version}/build/make.log." >&2
        failed=1
        continue
    fi
    echo "Installing NVIDIA %{version} for $kver..."
    if ! dkms install -m %{dkms_name} -v %{version} -k "$kver" --force --kernelsourcedir "$ksrc" --directive post_transaction=; then
        echo "NVIDIA module installation failed for $kver." >&2
        failed=1
    fi
done
if [ "$found" -eq 0 ]; then
    echo 'No installed kernels with headers found; NVIDIA modules have not been built.' >&2
fi
exit "$failed"

%preun
%{_libexecdir}/drm-awaiter-initramfs request || exit $?
# Keep the same version-specific removal semantics, but batch its image update.
dkms remove -m %{dkms_name} -v %{version} --all --rpm_safe_upgrade --directive post_transaction=
...
* Wed Oct 08 2025 Simone Caronni <negativo17@gmail.com> - 3:580.95.05-2
